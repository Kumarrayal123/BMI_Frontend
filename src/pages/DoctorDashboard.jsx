            ...
            <div className="space-y-10">
                ...
                <div className="admin-dash__stats">
                    ...
                    <div className="admin-dash__stat">
                        ...
                        <div className="admin-dash__stat-meta">created by you</div>
                    </div>
                    <div className="admin-dash__stat">
                        <div className="admin-dash__stat-top">
                                            ...
                                            <tr key={patient._id} className="transition-colors group hover:bg-gray-50/80">
                                                ...
                                                <td className="p-4">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <Link to={`/patient/${patient._id}`} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-orange-50 text-orange-700 hover:bg-orange-100 transition-colors">
                                                            <FiEdit size={14} /> Edit
                                                        </Link>
                ...
                <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="bg-white rounded-[40px] w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
                        <div className="p-8">
                            <div className="flex justify-between items-center mb-8">
                                <div>
                                    <h2 className="text-3xl font-black text-gray-900 tracking-tight">Edit Camp</h2>
                                    <p className="text-gray-500 font-medium">Update camp details</p>
                                </div>
                                <button onClick={() => { setShowEditModal(false); setEditingCamp(null); }} className="p-3 hover:bg-gray-100 rounded-2xl transition-colors">
                                    <FiXCircle className="text-gray-400" size={32} />
                                </button>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                <div className="space-y-2">
                                    <label className="text-sm font-black text-gray-700 uppercase tracking-widest ml-1">Camp Name</label>
                                    <input type="text" className="w-full px-6 py-4 bg-gray-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white rounded-2xl outline-none transition-all font-medium" value={campForm.name} onChange={(e) => setCampForm({ ...campForm, name: e.target.value })} />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-sm font-black text-gray-700 uppercase tracking-widest ml-1">Location</label>
                                    <input type="text" className="w-full px-6 py-4 bg-gray-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white rounded-2xl outline-none transition-all font-medium" value={campForm.location} onChange={(e) => setCampForm({ ...campForm, location: e.target.value })} />
                                </div>
                                <div className="space-y-2 md:col-span-2">
                                    <label className="text-sm font-black text-gray-700 uppercase tracking-widest ml-1">Full Address</label>
                                    <textarea rows="2" className="w-full px-6 py-4 bg-gray-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white rounded-2xl outline-none transition-all font-medium resize-none" value={campForm.address} onChange={(e) => setCampForm({ ...campForm, address: e.target.value })} />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-sm font-black text-gray-700 uppercase tracking-widest ml-1">Date</label>
                                    <input type="date" className="w-full px-6 py-4 bg-gray-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white rounded-2xl outline-none transition-all font-medium" value={campForm.date} onChange={(e) => setCampForm({ ...campForm, date: e.target.value })} />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-sm font-black text-gray-700 uppercase tracking-widest ml-1">Time</label>
                                    <input type="time" className="w-full px-6 py-4 bg-gray-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white rounded-2xl outline-none transition-all font-medium" value={campForm.time} onChange={(e) => setCampForm({ ...campForm, time: e.target.value })} />
                                </div>
                                <div className="space-y-2 md:col-span-2">
                                    <label className="text-sm font-black text-gray-700 uppercase tracking-widest ml-1 flex items-center gap-2">Assign Volunteers</label>
                                    <select onChange={handleAddVolunteer} className="w-full px-6 py-4 bg-gray-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white rounded-2xl outline-none transition-all font-medium" value="">
                                        <option value="">Choose Staff...</option>
                                        {volunteers.map(v => (
                                            <option key={v._id} value={v._id}>{v.name} ({v.designation || v.role || "Staff"})</option>
                                        ))}
                                    </select>
                                    <div className="flex flex-wrap gap-2 mt-3">
                                        {campForm.volunteers.map((vol, index) => (
                                            <span key={index} className="bg-indigo-50 text-indigo-700 px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 border border-indigo-100">
                                                {getVolunteerName(vol)}
                                                <button onClick={() => handleRemoveVolunteer(index)} className="hover:text-red-500"><FiXCircle size={16} /></button>
                                            </span>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="flex gap-4">
                                <button onClick={() => { setShowEditModal(false); setEditingCamp(null); }} className="flex-1 px-8 py-4 border-2 border-gray-100 text-gray-400 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-gray-50 transition-all">Cancel</button>
                                <button onClick={handleUpdateCamp} className="flex-[2] px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-200">Update Camp</button>
                            </div>
                        </div>
                    </div>
                </div>,
                                    ...
                                    {viewCampPatients.length > 0 ? (
                                        viewCampPatients.map(patient => (
                                            <tr key={patient._id} className="hover:bg-gray-50/80 transition-colors">
                                                ...
                                                <td className="p-4 text-right">
                                                    <Link to={`/patient/${patient._id}`} className="inline-flex items-center gap-1 text-xs font-bold text-indigo-600 hover:text-indigo-800 hover:underline">
                                                        Details <FiChevronRight size={12} />
                                                    </Link>
                                                </td>
                                            </tr>
                ...
                <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="bg-white w-full max-w-md rounded-3xl overflow-hidden shadow-2xl">
                        <div className="bg-green-600 p-6 text-white text-center">
                            ...
                            <h3 className="text-xl font-bold">Share Report Link</h3>
                            <p className="text-sm text-green-100 mt-1">WhatsApp message generated successfully!</p>
                        </div>
                        <div className="p-6 space-y-6">
                            <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl">
                                <div className="w-12 h-12 bg-green-100 text-green-700 rounded-full flex items-center justify-center font-bold text-lg">
                                ...
                                <div>
                                    <p className="font-bold text-gray-900">{currentPatient?.name}</p>
                                    <p className="text-xs text-gray-500">{currentPatient?.contact || "No Number"}</p>
                                </div>
                            </div>
                            <div className="p-4 bg-green-50/50 border-2 border-green-200 border-dashed rounded-2xl">
                                <p className="text-[10px] font-bold text-green-800 uppercase tracking-widest mb-2">Message Preview</p>
                                <div className="bg-white p-3 rounded-lg border border-green-100 max-h-32 overflow-y-auto text-xs text-gray-600">
                                    {downloadLink}
                                </div>
                            </div>
                            <div className="space-y-3">
