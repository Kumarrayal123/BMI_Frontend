...
const Dashboard = () => {
  ...
  const fetchCamps = async () => {
    try {
      ...
      // For partner role, fetch assigned camps
      if (role === "partner" && partnerId) {
        res = await axios.get(`${config.API_BASE_URL}/camps/assigned-camps/${partnerId}`);
      } else {
        // For admin or employee, fetch all camps
        res = await axios.get(`${config.API_BASE_URL}/camps/allcamps`);
      }

      let allCamps = res.data || [];
      if (!Array.isArray(allCamps)) {
        allCamps = allCamps.data || allCamps.camps || [];
        if (!Array.isArray(allCamps)) allCamps = [];
      }

      console.log("📦 All Camps:", allCamps.length);

      // For employee/volunteer, filter camps where they are assigned
      ...
      if (role === "employee" && employeeName) {
        filteredCamps = allCamps.filter(camp => {
          ...
          // Check if employee name matches any volunteer (case insensitive)
        ...
        console.log("📦 Employee Assigned Camps:", filteredCamps.length);
      }

      setCamps(sortCampsByStatus(filteredCamps));
    } catch (err) {
  ...
  // Get camps where employee is assigned
  const myAssignedCamps = useMemo(() => {
    if (role === "partner" && partnerId) {
      return camps; // Already filtered by API
    } else if (role === "employee" && employeeName) {
    ...
    return camps;
  }, [camps, role, employeeName, partnerId]);
  ...
  const filteredPatients = patients.filter((p) => {
    ...
    // For employee, only show patients from their assigned camps
    let matchCamp = true;
    if (selectedCampId === "all") {
      if (role === "employee" || role === "partner") {
        matchCamp = myAssignedCamps.some(c => String(c._id) === String(p.campId?._id));
      }
    } else {
      matchCamp = String(p.campId?._id) === String(selectedCampId);
    }
  ...
  const totalPatients = patients.filter(p => {
    if (role === "employee" || role === "partner") {
      return myAssignedCamps.some(c => String(c._id) === String(p.campId?._id));
    }
    return true;
  }).length;

  // Calculate Active and Upcoming Camps
  const activeCampsCount = camps.filter(c => {
    const { status } = getCampStatus(c.date, c.time);
    return status === 'live' || status === 'today';
  }).length;
  ...
  const viewCampPatients = useMemo(() => {
    if (!viewCamp) return [];
    return patients.filter(p => String(p.campId?._id) === String(viewCamp._id));
  }, [viewCamp, patients]);
  ...
  // Display camps based on role
  const displayCamps = useMemo(() => {
    if (role === "employee" && myAssignedCamps.length > 0) {
      return myAssignedCamps;
    }
    if (role === "partner" && myAssignedCamps.length > 0) {
      return myAssignedCamps;
    }
    return camps;
  }, [camps, myAssignedCamps, role]);
      ...
      <div className="admin-dash__wrapper">
        ...
        <div className="admin-dash__header">
          <div>
            ...
            <p className="admin-dash__subtitle">
              {role === "employee" 
                ? `Welcome, ${employeeName || 'Volunteer'}! Manage your assigned camps and patients.`
                : "Manage camps, patients, and health analytics in one place."}
            </p>
            {role === "employee" && (
            ...
            {role === "partner" && (
              <div className="mt-2 flex items-center gap-2 text-sm text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">
                <Activity size={14} />
                <span>Partner View - Showing your assigned camps only</span>
              </div>
            )}
          </div>
          <div className="admin-dash__date-pill">
        ...
        <div className="space-y-10">
          ...
          <div className="admin-dash__stats">
            <div className="admin-dash__stat" onClick={() => scrollToSection(campsSectionRef)}>
              ...
              <div className="admin-dash__stat-value">{displayCamps.length}</div>
              <div className="admin-dash__stat-meta">
                {role === "employee" ? "assigned camps" : "health camps"}
              </div>
            </div>
          ...
          <div ref={campsSectionRef} className="admin-dash__card">
            <div className="admin-dash__card-header">
              <h3 className="admin-dash__card-title">
                {role === "employee" ? "My Assigned Camps" : 
                 role === "partner" ? "My Assigned Camps" : 
                 "All Camps"}
              </h3>
              {displayCamps.length === 0 && role === "employee" && (
                <span className="text-sm text-gray-400">No camps assigned to you yet.</span>
              )}
            </div>
            <div className="admin-dash__card-body">
                  ...
                  {displayCamps.map((camp) => (
                    ...
                    >
                      ...
                      <div className="space-y-2 text-sm font-medium">
                        ...
                        {/* Creator Info */}
                        <div className={`text-[10px] font-semibold mt-2 ${selectedCampId === camp._id ? "text-indigo-100" : "text-gray-500"}`}>
                        ...
                        {camp.assignedPartner && (
                          <div className={`text-[10px] font-semibold mt-1 ${selectedCampId === camp._id ? "text-indigo-100" : "text-gray-500"}`}>
                  ...
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-3 text-xs">
                      ...
                      {viewCamp.assignedPartner && (
                        <div className="flex items-center gap-1 text-indigo-700 bg-indigo-50 px-2.5 py-1 rounded-lg border border-indigo-100">
